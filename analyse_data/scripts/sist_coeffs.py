...
def get_syst_estimate( data, param ):
    """
    Function to return a systematic estimation of the value and 
    the error given a scanned fit of the data. The function 
    histograms all the data and picks the most repeated values, 
    then it calculates the error using,

                    0.5 *  ( Maximum - Minimum ).
    In the case in which the data is unstable, then it will 
    retrieve a value corresponding to the average of the input 
    data. The error will then be the standard deviation of the data.

    Arguments:
        data ( np.array ):
            Array containing the data, the first column has to 
            be the initial time that defines the time window, the 
            second is the mass, third its error.
            If you do void fitting, then the fourth colum is the 
            void and fifth is its error.
        param ( string = ['mass', void'] ):
            String that select which column to analyse in this run.
    Returns:
        estimation ( list ):
            List containing the value and the error estimated in 
            the calculation.
    """
    ...
    # Sort the histograms according to number of bins in each case
    ...
    sys_error = 0.5 * ( np.max( check_param ) - \
        np.min( check_param ) )
    # Statistical error as the mean of standard deviations
    ...
    if sys_error > stat_error:
        ...
        # Take care of only one point in the histogram
        if sys_error == 0:  
            new_error = 0.5 * ( np.max( data[:,col] ) - \
                np.min( data[:,col] ) )
